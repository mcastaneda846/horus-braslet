...
export default function OrderHistoryModal({ orders, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div className="relative bg-[var(--h-card)] rounded-[28px] shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-7 py-5 border-b border-[var(--h-border)]">
          <div>
            <h2 className="font-display text-xl font-bold text-[var(--h-text)]">Historial de compras</h2>
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-[var(--h-bg)] hover:bg-[var(--h-border)] flex items-center justify-center transition-colors"
          >
            <svg className="w-4 h-4 text-[var(--h-text)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        <div className="overflow-y-auto flex-1 px-7 py-5 space-y-3">
          {orders.length === 0 ? (
            <div className="py-16 flex flex-col items-center gap-3 text-center">
              <div className="w-14 h-14 rounded-full bg-[var(--h-bg)] flex items-center justify-center">
              ...
              <p className="text-sm font-semibold text-[var(--h-text)]">Aún no has comprado</p>
            ...
            orders.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className="bg-[var(--h-card2)] rounded-[18px] p-5 flex flex-col sm:flex-row sm:items-start gap-4"
                >
                  {/* Ícono */}
                  <div className="w-11 h-11 rounded-xl bg-[var(--h-card)] border border-[var(--h-border)] flex items-center justify-center flex-shrink-0 shadow-sm">
                    {isCard ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className="text-sm font-bold text-[var(--h-text)]">{order.product?.name ?? "Horus"}</span>
                      <span
                    ...
                    <p className="text-[11px] text-[#8D99AE] mt-0.5">
                      ...
                      {order.subscription?.endDate && (
                        <> · <span className="text-[var(--h-text)] font-medium">Vigente hasta {new Date(order.subscription.endDate).toLocaleDateString("es-CO")}</span></>
                      )}
                    </p>
                      ...
                      <div className="flex gap-2 mt-2">
                        {order.cardFrontUrl && (
                          <div className="relative">
                            <img src={order.cardFrontUrl} alt="Frontal" className="w-10 h-10 rounded-lg object-cover border border-[var(--h-border)]" />
                            <span className="absolute -bottom-1 -right-1 text-[9px] bg-[var(--h-dark)] text-[var(--h-bg)] px-1 rounded">F</span>
                          </div>
                        )}
                        {order.cardBackUrl && (
                          <div className="relative">
                            <img src={order.cardBackUrl} alt="Trasero" className="w-10 h-10 rounded-lg object-cover border border-[var(--h-border)]" />
                            <span className="absolute -bottom-1 -right-1 text-[9px] bg-[var(--h-dark)] text-[var(--h-bg)] px-1 rounded">T</span>
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-base font-bold text-[var(--h-text)]">
