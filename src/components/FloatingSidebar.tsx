...
export default function FloatingSidebar() {
    ...
    const items: NavItem[] = [
        ...
        },
        {
            label: "Tienda",
            href: "/tienda",
            activeBg: "bg-[#F3E5F5]", // Soft purple
            activeFg: "text-[#4A148C]",
            icon: (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    const sidebarContent = (
        <div className="flex flex-col h-full">
            ...
            <nav className="flex flex-col gap-2 flex-1">
                {items.map((item) => {
                    const isActive = item.href === "/tienda"
                        ? (pathname === "/tienda" || pathname.startsWith("/personalizar") || pathname.startsWith("/checkout"))
                        : pathname === item.href;
                    return (
                        <Link
                            key={item.label}
